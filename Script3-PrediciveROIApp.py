...
#show_data = st.checkbox("Show Raw Numbers")
#if show_data:
#    st.write(july)

#To be added as the very last step

#st.header("Clicks by Timezone")

#newdf = session.sql("""select postal, city,timezone,count(*) as page_views, sum(click) as clicks from dcwt.schemaxx.click_data a
#,IP_GEOLOCATION.DEMO.LOCATION b where substring(start_ip,1,10) = substring(ipaddress,1,10)
#group by postal, city,timezone order by count(*) desc;""").toPandas()

#select_timezone = st.multiselect("Select Timezone", newdf.TIMEZONE.unique())
#newdf = newdf[newdf.TIMEZONE.isin(select_timezone)]

#st.bar_chart(newdf, x='CITY',y='CLICKS')
